...
import { generatePersonasWithGemini } from "@/lib/gemini";
  ...
  try {
    ...
    if (!isValidSurveyPayload(body.survey)) {
      return NextResponse.json(
        {
          error:
            "Invalid survey payload. Expected owner + devices[] with likeScore, frequency, helpfulnessPercent, and easeOfUseScore.",
        },
        { status: 400 },
      );
    }

    const response = await generatePersonasWithGemini({
      survey: body.survey,
      model: body.model,
    });
