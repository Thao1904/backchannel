...
type LikeVerticalSliderProps = {
  value?: number;
  onChange: (value: number) => void;
};

export function LikeVerticalSlider({
  value,
  onChange,
}: LikeVerticalSliderProps) {
  ...
  const scaleY = 1 + Math.abs(overflow) / 180;
  const heartScale = typeof value === "number" ? 0.92 + value * 0.14 : 1.2;
  const heartOpacity = typeof value === "number" ? 1 : 0.28;
      ...
      <div className="relative flex h-[360px] w-[122px] flex-col items-center justify-center">
        ...
        >
          <div className="absolute inset-[4px] overflow-hidden rounded-[14px]">
            <div
              className="absolute bottom-0 left-0 right-0 rounded-[12px] bg-[#ff4fc0] shadow-[inset_0_0_0_2px_rgba(24,21,21,0.08)] transition-[height,transform] duration-200 ease-out"
              style={{
                height: fillHeight,
          ...
          />
          <div
            className="pointer-events-none absolute right-full mr-5 top-1/2 h-[78px] w-[78px] transition-all duration-300 ease-out"
            style={{
